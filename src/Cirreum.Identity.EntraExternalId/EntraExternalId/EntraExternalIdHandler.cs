...
internal sealed partial class EntraExternalIdHandler(
	...
	public async Task<IResult> HandleAsync(HttpRequest request, CancellationToken cancellationToken = default) {
		...
		using var trace = ProvisioningTelemetry.StartProvision("entra", settings.Source);
		...
		if (logger.IsEnabled(LogLevel.Information)) {
			var claims = string.Join(", ", claimMap.Keys);
			Log.IssuingClaims(logger, claims, provisionContext.ExternalUserId, context.CorrelationId, settings.Source);
		}
			...
			Data = new EntraResponseData {
				Actions = [
					new EntraTokenAction {
						Claims = EntraClaimsPayload.From(context.CorrelationId, claimMap)
					}
				]
			}
		};
